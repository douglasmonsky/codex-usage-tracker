...
Use local pattern scans (`usage_repetition_scan`, `usage_command_loop_scan`, `usage_file_churn_scan`, `usage_context_bloat_scan`) when user asks for recurring waste, command loops, repeated file churn, or context bloat. Use `usage_content_search` only when user asks local content exploration, pattern hunting, or diagnostics need indexed snippets; its payload can include local transcript snippets is not default shareable report.
