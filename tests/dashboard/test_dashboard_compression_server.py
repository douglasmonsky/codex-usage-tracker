...
def test_dashboard_compression_routes_share_persistent_application_job(
    ...
    missing_payload = cast(dict[str, Any], missing["payload"])
    assert missing_payload["schema"] == "codex-usage-tracker-compression-api-v1"
    assert missing_payload["error"]["code"] == "compression_run_not_found"
