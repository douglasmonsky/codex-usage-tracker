...
def test_refresh_payload_preserves_nonzero_parser_diagnostics() -> None:
    result = RefreshResult(
        scanned_files=1,
        parsed_events=1,
        inserted_or_updated_events=1,
        db_path="/synthetic/usage.sqlite3",
        parser_diagnostics={"unknown_events": 1},
    )

    payload = refresh_result_payload(result, schema="synthetic-refresh-v1")

    assert payload["parser_diagnostics"] == {"unknown_events": 1}
