...
def test_common_query_plans_use_allowance_indexes_without_temp_ordering() -> None:
    conn = seeded_connection()
    latest = [
        row["detail"]
        for row in conn.execute(
            "EXPLAIN QUERY PLAN SELECT * FROM allowance_cycles "
            "WHERE is_archived = ? AND window_kind = ? AND cohort_key = ? "
    ...
    assert any("idx_allowance_cycles_latest_cohort_window" in detail for detail in latest)
    assert any("idx_allowance_intervals_cycle_evidence_desc" in detail for detail in evidence)
    assert not any("USE TEMP B-TREE" in detail for detail in latest + evidence)
