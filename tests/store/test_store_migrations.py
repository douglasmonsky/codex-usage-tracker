...
def test_init_db_records_all_schema_migrations_for_new_database(tmp_path: Path) -> None:
    ...
    with connect(db_path) as conn:
        ...
        allowance_interval_columns = {
            row["name"] for row in conn.execute("PRAGMA table_info(allowance_intervals)").fetchall()
        }
    ...
    assert {
        ...
        "idx_recommendation_facts_time_model",
        "idx_recommendation_facts_time_effort",
    } <= recommendation_indexes
    ...
    assert {
        "idx_allowance_observations_all_newest",
    ...
    assert "idx_allowance_intervals_source_revision" not in interval_indexes
    ...
    assert any(
        "idx_allowance_observations_active_newest" in detail for detail in allowance_newest_plan
    )
    assert any(
    ...
    assert any(
        "idx_allowance_observations_all_newest" in detail for detail in allowance_all_newest_plan
    )
    ...
    assert not any("USE TEMP B-TREE" in detail for detail in allowance_all_newest_plan)
    assert {
...
def test_init_db_upgrades_v27_allowance_indexes_to_v28(tmp_path: Path) -> None:
    ...
    with connect(db_path) as conn:
        init_db(conn)
        indexes = {row["name"] for row in conn.execute("PRAGMA index_list(allowance_intervals)")}
        versions = [
            row["version"]
...
def test_init_db_upgrades_v28_with_allowance_plan_provenance(tmp_path: Path) -> None:
    ...
    with connect(db_path) as conn:
        init_db(conn)
        columns = {row["name"] for row in conn.execute("PRAGMA table_info(allowance_cycles)")}
        versions = [
            row["version"]
            for row in conn.execute("SELECT version FROM schema_migrations ORDER BY version")
        ]
...
def test_init_db_upgrades_v31_with_all_history_allowance_index(tmp_path: Path) -> None:
    db_path = tmp_path / "usage.sqlite3"
    with connect(db_path) as conn:
        init_db(conn)
...
def test_init_db_upgrades_v33_with_focused_call_indexes_without_data_loss(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "usage.sqlite3"
    upsert_usage_events(
        [_event("migration-34-sentinel", "/synthetic/migration-34.jsonl")],
        db_path=db_path,
    )
    expected_indexes = {
        "idx_recommendation_facts_thread_latest",
        "idx_recommendation_facts_time_model",
        "idx_recommendation_facts_time_effort",
        "idx_recommendation_facts_attention_sort",
        "idx_recommendation_facts_cost_sort",
        "idx_recommendation_facts_credits_sort",
        "idx_recommendation_facts_context_sort",
        "idx_recommendation_facts_pricing_coverage_scope",
        "idx_recommendation_facts_pricing_estimated_scope",
        "idx_recommendation_facts_credit_confidence_scope",
        "idx_recommendation_facts_record_filter_cover",
        "idx_usage_parent_thread_lookup",
        "idx_usage_parent_updated_lookup",
        "idx_usage_cwd_scope",
    }
    with connect(db_path) as conn:
...
def test_init_db_upgrades_pre_release_v36_analysis_jobs_with_expired_leases(
    tmp_path: Path,
) -> None:
    db_path = tmp_path / "usage.sqlite3"
    with connect(db_path) as conn:
        init_db(conn)
        conn.execute("ALTER TABLE analysis_jobs DROP COLUMN owner_id")
        conn.execute("ALTER TABLE analysis_jobs DROP COLUMN lease_expires_at")
        conn.execute(
            """
            INSERT INTO analysis_jobs (
                job_id, job_kind, semantic_key, status, source_revision,
                request_schema, request_json, progress_json, result_schema,
    ...
    with connect(db_path) as conn:
        ...
        migrated = conn.execute(
            "SELECT owner_id, lease_expires_at FROM analysis_jobs WHERE job_id = ?",
            ("pre-release-job",),
        ).fetchone()
        user_version = conn.execute("PRAGMA user_version").fetchone()[0]
    ...
    assert migrated["lease_expires_at"] == "1970-01-01T00:00:00Z"


def test_init_db_upgrades_v37_allowance_indexes_without_data_loss(tmp_path: Path) -> None:
    db_path = tmp_path / "usage.sqlite3"
    with connect(db_path) as conn:
        init_db(conn)
        conn.executescript(
            ...
            ) VALUES ('migration-39-cycle', 'weekly', 'primary', 'codex', 'revision');
            INSERT INTO allowance_intervals (
                interval_id, cycle_id, window_kind, window_key, cohort_key,
                point_kind, source_revision
            ) VALUES (
                'migration-39-interval', 'migration-39-cycle', 'weekly',
                'primary', 'codex', 'positive', 'revision'
            );
            DROP INDEX idx_allowance_intervals_evidence_cohort_window;
            DROP INDEX idx_allowance_intervals_evidence_window;
            ...
            ON allowance_intervals(
                is_archived, source_revision, window_kind,
                end_observed_at DESC, interval_id DESC
            );
    ...
    with connect(db_path) as conn:
        init_db(conn)
        indexes = {
            str(row["name"]) for row in conn.execute("PRAGMA index_list(allowance_intervals)")
        }
        evidence_columns = [
            str(row["name"])
            for row in conn.execute(
                "PRAGMA index_info(idx_allowance_intervals_evidence_cohort_window)"
            )
        ]
        interval = conn.execute(
            "SELECT interval_id FROM allowance_intervals "
            "WHERE interval_id = 'migration-39-interval'"
        ).fetchone()
        user_version = conn.execute("PRAGMA user_version").fetchone()[0]
