...
def _canonical_cbor(value: Any) -> bytes:
    ...
    if isinstance(value, dict):
        pairs = [
            (_canonical_cbor(key), _canonical_cbor(item))
...
def _coordinate_key(fact: Mapping[str, Any]) -> tuple[Any, ...]:
    ...
    return (at is None, at or 0, source_rank, source_order, event_kind_order, logical_id, transition_rank)
...
def _scoped(rows: Iterable[Mapping[str, Any]], request: Mapping[str, Any], name: str = "window") -> list[Mapping[str, Any]]:
...
def _calls(case: Mapping[str, Any], request: Mapping[str, Any], name: str = "window") -> list[Mapping[str, Any]]:
...
def _total(rows: Sequence[Mapping[str, Any]], *, empty: Any = None) -> Any:
    ...
    if any(value is None for value in values):
        return None
    return sum((Decimal(value) for value in values if value is not None), Decimal(0)) if any(isinstance(value, Decimal) for value in values) else sum(values)
...
def _top_sessions(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    denominator = sum((Decimal(value) for value in totals.values() if value is not None), Decimal(0))
...
def _period_drivers(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    field = {"model_profile": "model_profile_id", "project": "project_id", "session": "session_id", "tool": "tool_id"}.get(dimension)
    if field is None:
    ...
    for key, rows in groups.items():
        ...
        out.append({"current_total_tokens": now, "driver_contribution": 0 if delta is None else delta, "driver_id": key, "previous_total_tokens": prior, "total_delta": delta})
    return sorted(
        out,
        key=lambda row: (
...
def _project_family(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for key, rows in groups.items():
        family = _total(rows)
        out.append({"exclusive_tokens": family, "family_id": key, "family_tokens": family, "session_count": len({_value(row, "session_id") for row in rows}), "share": _ratio(family, denominator)})
    return sorted(out, key=lambda row: (-(Decimal(row["family_tokens"]) if row["family_tokens"] is not None else 0), str(row["family_id"])))
...
def _top_valued(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for key, rows in groups.items():
        ...
        out.append({"configured_cost_usd": cost, "entity_id": key, "estimated_credits": cost, "rated_calls": len(rated), "total_tokens": _total(rows), "unrated_calls": len(values) - len(rated)})
    return sorted(out, key=lambda row: (-(Decimal(row["configured_cost_usd"]) if row["configured_cost_usd"] is not None else 0), str(row["entity_id"])))
...
def _pricing_coverage(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for key, rows in groups.items():
        ...
        out.append({"model_profile_id": key, "priced_calls": len(priced), "pricing_coverage": _ratio(len(priced), len(rows)), "unpriced_calls": len(unpriced), "unpriced_tokens": _total(unpriced)})
    return sorted(out, key=lambda row: (-(Decimal(row["unpriced_tokens"]) if row["unpriced_tokens"] is not None else 0), str(row["model_profile_id"])))
...
def _cache_reuse(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in _calls(case, request):
        ...
        out.append({"cached_input_tokens": cached, "cached_share": _ratio(cached, (Decimal(cached or 0) + Decimal(uncached or 0)) if cached is not None or uncached is not None else None), "call_id": _value(row, "call_id"), "total_input_tokens": total_input, "uncached_input_tokens": uncached})
    return sorted(out, key=lambda row: (-(Decimal(row["total_input_tokens"]) if row["total_input_tokens"] is not None else 0), str(row["call_id"])))
...
def _context_pressure(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in calls:
        ...
        preceding = [boundary for boundary in boundaries if _value(boundary, "session_id") == session and _coordinate_key(boundary) <= _coordinate_key(row)]
    ...
    for (session, epoch), rows in groups.items():
        ...
        out.append({"cached_share": _ratio(cached, (Decimal(cached or 0) + Decimal(uncached or 0)) if cached is not None or uncached is not None else None), "context_epoch_id": epoch, "context_pressure": _ratio(last_input, _value(last, "context_window_tokens")), "context_window_tokens": _value(last, "context_window_tokens"), "ordered_input_tokens": ordered_inputs, "session_id": session, "_sort": -(Decimal(last_input) if last_input is not None else 0)})
    out.sort(key=lambda row: (row.pop("_sort"), str(row["session_id"]), str(row["context_epoch_id"])))
    return out


def _token_acceleration(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    calls = _calls(case, request)
    ...
    for session, rows in groups.items():
        ...
        out.append({"later_earlier_ratio": _ratio(sum((Decimal(value or 0) for value in later), Decimal(0)), sum((Decimal(value or 0) for value in earlier), Decimal(0))), "second_difference": acceleration, "session_id": session, "turn_tokens": totals, "_sort": -acceleration})
...
def _uncached_jumps(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for previous, current in zip(rows, rows[1:], strict=False):
        ...
        out.append({"call_id": _value(current, "call_id"), "current_uncached_input_tokens": current_value, "input_delta": _difference(current_value, previous_value), "previous_uncached_input_tokens": previous_value})
    return out
...
def _cached_replay(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for row in _calls(case, request):
        ...
        if output is None or Decimal(output) > Decimal(threshold):
            continue
        cached = _number(_value(row, "cached_input_tokens"), "cached_input_tokens")
        out.append({"cached_input_tokens": cached, "cached_output_ratio": _ratio(cached, output), "call_id": _value(row, "call_id"), "output_tokens": output})
    return sorted(out, key=lambda row: (-(Decimal(row["cached_input_tokens"]) if row["cached_input_tokens"] is not None else 0), str(row["call_id"])))


def _context_composition(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for category, rows in groups.items():
        ...
        out.append({"component_bytes": observed, "component_count": len(rows), "component_kind": category, "estimated_component_tokens": _sum_field(rows, "estimated_tokens"), "unattributed_bytes": None if total is None else total - observed})
    return sorted(out, key=lambda row: (-row["component_bytes"], str(row["component_kind"])))


def _compaction_comparison(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    calls = _calls(case, request)
    out = []
    for boundary in _ordered(_scoped(_facts(case, "compaction_boundary"), request)):
        ...
        before = [
            row
            for row in calls
            if isinstance((row_at := _coordinates(row).get("event_at_us")), int)
            and row_at < at
        ]
        after = [
            row
        ...
        out.append({"after_tokens": after_value, "before_tokens": before_value, "compaction_id": _value(boundary, "compaction_id"), "context_delta": _difference(after_value, before_value)})
    return out
...
def _growth(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for session, rows in groups.items():
        ...
        out.append({"context_growth": _difference(last, first), "mutation_count": sum(_value(row, "session_id") == session for row in changes), "session_id": session, "turn_count": len({_value(row, "turn_id") for row in turns if _value(row, "session_id") == session}), "_sort": -(Decimal(_difference(last, first) or 0))})
    out.sort(key=lambda row: (row.pop("_sort"), str(row["session_id"])))
    return out
...
def _long_cohorts(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for side in ("left", "right"):
        ...
        out.append({"cohort": side, "cohort_size": len(ids), "context_features": {"total_context_tokens": _total(rows)}, "usage_features": {"total_tokens": _total(rows)}})
    return out
...
def _turn_completion(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for session, rows in groups.items():
        ...
        total = _total(rows)
        out.append({"calls": len(rows), "completion_state": "completed" if session_turns and len(completed) == len(session_turns) else "incomplete", "session_id": session, "tokens_per_completed_turn": _ratio(total, len(completed)), "total_tokens": total, "turns": len(session_turns), "_sort": -len(rows)})
    out.sort(key=lambda row: (row.pop("_sort"), str(row["session_id"])))
    return out


def _first_action(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    """Apply R1A boundaries to structural-v2's one-coordinate tool facts.

    The standalone R1A vector evaluator below handles explicit start and
    terminal coordinates.  Structural-v2 declarations predate that split and
    represent an invocation at one coordinate, so a successful invocation is
    treated as a coincident start/terminal event for this compatibility seam.
    """

    calls = _calls(case, request)
    tools = _ordered(_scoped(_facts(case, "tool_invocation"), request))
    changes = _ordered(_scoped(_facts(case, "state_change"), request))
    action = tools[0] if tools else None
    success = next((tool for tool in tools if _value(tool, "lifecycle") == "succeeded"), None)
    mutation = changes[0] if changes else None
    def before(boundary: Mapping[str, Any] | None) -> int | None:
        if boundary is None:
            return None
        prior = [call for call in calls if _coordinate_key(call) < _coordinate_key(boundary)]
        return _total(prior, empty=0)
    return [{"first_action_tokens": before(action), "first_mutation_tokens": before(mutation), "first_success_tokens": before(success), "mutation_observed": mutation is not None}]
...
def _repeated_resources(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    groups = _group(_ordered(_scoped(_facts(case, "tool_invocation"), request)), "resource_id")
    out = []
    for resource, rows in groups.items():
        out.append({"operation_count": len(rows), "resource_id": resource, "revisit_count": max(0, len(rows) - 2), "revisit_distance": []})
...
def _retry_cycles(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for resource, rows in groups.items():
        ...
        out.append({"matched_events": matched, "resource_id": resource, "retry_cycles": cycles, "terminal_status": stages[-1] if stages else None})
    return out
...
def _tool_family(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for family, rows in groups.items():
        following = [call for call in calls if any(_coordinate_key(call) > _coordinate_key(tool) for tool in rows)]
        following_tokens = _total([following[0]]) if following else None
        out.append({"calls": len(rows), "duration_us": _sum_field(rows, "duration_us"), "failure_count": sum(_value(row, "lifecycle") == "failed" for row in rows), "following_tokens": None if following_tokens is None else str(following_tokens), "tool_name": family, "tool_output_bytes": _sum_field(rows, "output_bytes"), "_sort": -len(rows)})
...
def _following_tool(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for tool in tools:
        ...
        if not later:
            continue
        following = later[0]
        out.append({"following_cached_input_tokens": _value(following, "cached_input_tokens"), "following_output_tokens": _value(following, "output_tokens"), "following_uncached_input_tokens": _value(following, "uncached_input_tokens"), "intervening_events": [0], "tool_id": _value(tool, "tool_id"), "tool_output_bytes": _value(tool, "output_bytes")})
    return sorted(out, key=lambda row: (-row["tool_output_bytes"], str(row["tool_id"])))[:1]


def _tool_output_adjacency(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for tool in tools:
        ...
        out.append({
            "following_input_delta": _difference(
                ...
                _value(calls[calls.index(following) - 1], "uncached_input_tokens") if calls.index(following) else None,
            ),
...
def _resource_hotspots(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for resource, rows in groups.items():
        ...
        out.append({"duration_us": _sum_field(rows, "duration_us"), "observed_mutations": sum(_value(row, "resource_id") == resource for row in changes), "operation_count": dict(sorted(operation_count.items())), "resource_id": resource, "tool_output_bytes": _sum_field(rows, "output_bytes")})
    return sorted(out, key=lambda row: (-sum(row["operation_count"].values()), str(row["resource_id"])))


def _profile_transitions(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    groups = _group(_calls(case, request), "session_id")
    out = []
    for rows in groups.values():
        for previous, current in zip(rows, rows[1:], strict=False):
            if _value(previous, "model_profile_id") == _value(current, "model_profile_id"):
                continue
            out.append({"current_profile": _value(current, "model_profile_id"), "previous_profile": _value(previous, "model_profile_id"), "token_delta": [_difference(_total([current]), _total([previous]))], "transition_count": 1, "transition_id": _value(current, "call_id")})
...
def _automation(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for tool in tools:
        groups[(_value(tool, "semantic_operation"), _value(tool, "resource_kind"), _value(tool, "write_intent"))].append(tool)
    out = []
    for signature, rows in groups.items():
        ...
        out.append({"failure_coverage": _ratio(sum(_value(row, "lifecycle") == "failed" for row in rows), len(rows)), "feature_id": feature_id, "frequency": len(rows), "mutation_coverage": _ratio(mutation_count, len(rows)), "structural_features": {"operation": signature[0], "resource_kind": signature[1], "sequence_count": 1, "write_intent": signature[2]}})
...
def _parent_subagent(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for parent in sorted(parent_ids):
        ...
        descendant_total = _total([row for row in calls if _value(row, "session_id") in descendants])
        out.append({"child_count": len(direct_children), "descendant_exclusive_tokens": descendant_total, "family_inclusive_tokens": family_total, "parent_exclusive_tokens": _difference(family_total, descendant_total), "session_id": parent})
    return sorted(out, key=lambda row: (-(Decimal(row["family_inclusive_tokens"]) if row["family_inclusive_tokens"] is not None else 0), str(row["session_id"])))


def _delegation_cohorts(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    cohort = request.get("parameters", {}).get("cohort", {})
    calls = _calls(case, request)
    tools = _scoped(_facts(case, "tool_invocation"), request)
    changes = _scoped(_facts(case, "state_change"), request)
    direct: dict[str, Any] = {"cohort_size": {}, "model_mix": {}, "mutation_features": {}, "usage_features": {}}
    for side in ("left", "right"):
        ids = cohort.get(side, []) if isinstance(cohort, Mapping) else []
        rows = [row for row in calls if _value(row, "session_id") in set(ids)]
        direct["cohort_size"][side] = len(ids)
        direct["model_mix"][side] = {}
        direct["mutation_features"][side] = {"state_changes": sum(_value(change, "session_id") in ids for change in changes), "tools": sum(_value(tool, "session_id") in ids for tool in tools)}
        direct["usage_features"][side] = {"calls": len(rows), "tokens": _total(rows)}
    return [direct]


def _observations(case: Mapping[str, Any]) -> list[Mapping[str, Any]]:
    return sorted(_facts(case, "allowance_observation"), key=lambda row: (_value(row, "observed_at_us"), str(_value(row, "observation_id"))))


def _intervals(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[tuple[Mapping[str, Any], Mapping[str, Any], list[Mapping[str, Any]]]]:
...
def _interval_direct(case: Mapping[str, Any], rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    return {"calls": len(rows), "turns": len({_value(row, "turn_id") for row in rows}), "cached_input_tokens": _sum_field(rows, "cached_input_tokens"), "uncached_input_tokens": _sum_field(rows, "uncached_input_tokens"), "output_tokens": _sum_field(rows, "output_tokens"), "reasoning_tokens": _sum_field(rows, "reasoning_tokens"), "configured_cost_usd": _cost_sum(case, rows), "estimated_credits": _cost_sum(case, rows)}


def _allowance_movement(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    out = []
...
def _allowance_efficiency(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    out = []
    for left, right, rows in _intervals(case, request):
        ...
        out.append({"allowance_delta": delta, "allowance_interval_id": interval_id, "calls_per_point": _ratio(direct["calls"], delta), "cost_per_point": _ratio(direct["configured_cost_usd"], delta), "credits_per_point": _ratio(direct["estimated_credits"], delta), "tokens_per_point": _ratio(_total(rows), delta), "turns_per_point": _ratio(direct["turns"], delta)})
    return out
...
def _allowance_cycles(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for left, right, rows in _intervals(case, request):
        ...
        out.append({"allowance_interval_id": f"{_value(left, 'observation_id')}:{_value(right, 'observation_id')}", "allowance_movement": _scalar(movement), "calls": direct["calls"], "configured_cost_usd": direct["configured_cost_usd"], "cycle_duration_us": _value(right, "observed_at_us") - _value(left, "observed_at_us"), "estimated_credits": direct["estimated_credits"], "total_tokens": _total(rows, empty=0)})
    return out
...
def _publication_delta(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if not facts:
        return []
    return [{name: _value(facts[0], name) for name in ("corrected_count", "inserted_count", "recanonicalized_count", "removed_count", "terminalized_count", "token_delta")}]
...
def _data_health(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [{"capabilities": _value(publication, "capabilities"), "freshness_age_us": as_of - _value(publication, "observed_through_us"), "guaranteed_complete_from_us": _value(publication, "guaranteed_complete_from_us"), "indexed_from_us": _value(publication, "indexed_from_us"), "measurements": _value(publication, "measurements"), "valuation_coverage": _value(publication, "valuation_coverage")}]
...
def _dedup(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    call_ids = {str(row.get("logical_id")) for row in calls} | {str(_value(row, "call_id")) for row in calls}
    ...
    for manifestation in _facts(case, "source_manifestation"):
        ...
        semantic_ids = {_value(row, "semantic_logical_id") for row in linked if _value(row, "semantic_logical_id") in call_ids}
        out.append({"canonical_basis": _value(manifestation, "canonical_basis"), "excluded_occurrence_count": max(0, len(linked) - len(semantic_ids)), "manifestation_count": 1, "semantic_entity_count": len(semantic_ids), "source_manifestation_id": mid, "_sort": -len(linked)})
...
def _evidence_timeline(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    allowed = {"source_occurrence", "turn", "allowance_observation", "canonical_call", "tool_invocation", "state_change", "resource"}
    ...
    for fact in selected:
        ...
        if at is None and str(fact.get("relation")) == "source_occurrence":
            ...
            semantic_coordinates = semantic_fact.get("coordinates") if isinstance(semantic_fact, Mapping) else None
            at = semantic_coordinates.get("event_at_us") if isinstance(semantic_coordinates, Mapping) else None
        if at is None and str(fact.get("relation")) == "source_occurrence":
            continue
        if not isinstance(at, int):
            at = None
        values = fact.get("values", {})
        occurrence = values.get("occurrence_coordinates") if str(fact.get("relation")) == "source_occurrence" else None
        record_ordinal = occurrence.get("record_ordinal", 0) if isinstance(occurrence, Mapping) else 0
        source_order = coordinates.get("source_order", 0) if isinstance(coordinates, Mapping) else 0
        event_kind_order = coordinates.get("event_kind_order", 0) if isinstance(coordinates, Mapping) else 0
        events.append({"event_kind": _event_kind(str(fact.get("relation"))), "event_time_us": at, "lifecycle_basis": values.get("lifecycle") if isinstance(values, Mapping) and str(fact.get("relation")) in {"turn", "tool_invocation", "canonical_call"} else None, "logical_id": fact.get("logical_id"), "occurrence_coordinates": occurrence, "token_measurements": None, "_sort": (at is None, at if at is not None else 0, relation_rank[str(fact.get("relation"))], record_ordinal, source_order, event_kind_order, str(fact.get("logical_id")))})
...
def _weekly_review(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    return [{"allowance_facts": {"observations": len(_facts(case, "allowance_observation"))}, "change_drivers": None, "context_facts": {"calls": len(calls)}, "model_mix": {"profiles": sorted({_value(row, "model_profile_id") for row in calls})}, "session_concentration": None, "tool_mix": {"tools": len(tools)}, "usage_totals": _total(calls, empty=0)}]
...
def _investigation(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    tools = _scoped(_facts(case, "tool_invocation"), request)
    changes = _scoped(_facts(case, "state_change"), request)
    return [{"baseline": {"call_count": len(calls)}, "candidate_features": {"calls": len(calls), "state_changes": len(changes), "tools": len(tools)}, "coverage": {"state_change_count": len(changes), "tool_count": len(tools)}, "representative_selectors": {"session_ids": sorted({_value(row, "session_id") for row in calls})}}]
...
def _session_calls(case: Mapping[str, Any], request: Mapping[str, Any], session_id: str) -> list[Mapping[str, Any]]:
...
def _compare_sessions(case: Mapping[str, Any], request: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if left_id not in session_rows or right_id not in session_rows or len(session_rows) != len(sessions):
    ...
    if publications:
        ...
        context_available = capabilities.get("structural_context", capabilities.get("context_components"))
    ...
    for side, sid in zip(("left", "right"), selected, strict=True):
        ...
        if context_available:
            ...
            for call in side_calls:
                ...
                if isinstance(measurement_mask, Mapping) and measurement_mask.get("context_window_tokens") is False:
                ...
                values.append(value)
            context = {"observed_call_count": len(side_calls), "distinct_context_window_tokens": sorted(set(values))}
        result[f"{side}_completion"] = {"lifecycle_state": _value(session, "lifecycle_state"), "completion_basis": _value(session, "completion_basis")}
        ...
        inclusive = None if exclusive is None or descendant_total is None else exclusive + descendant_total
        result[f"{side}_delegation"] = {"exclusive_tokens": exclusive, "descendant_tokens": descendant_total, "inclusive_tokens": inclusive}
        ...
        for tool in side_tools:
            ...
            if links is not None:
                if not isinstance(links, list) or not all(isinstance(item, str) and item for item in links):
                    raise SemanticError("resource_links are malformed")
                linked.update(links)
        linked.update(str(_value(change, "resource_id")) for change in side_changes if _value(change, "resource_id", required=False) is not None)
        ...
        for resource_id in linked:
            ...
            by_kind[kind] += 1
        result[f"{side}_resources"] = {"count": len(linked), "by_kind": dict(sorted(by_kind.items()))}
        ...
        result[f"{side}_changes"] = {"count": len(change_ids), "by_mutation_kind": dict(sorted(mutation_kinds.items()))}
        ...
        counts = {"invocation_count": len(tool_ids), "succeeded_count": 0, "failed_count": 0, "open_count": 0}
        for tool in side_tools:
            lifecycle = _value(tool, "lifecycle")
    ...
    deltas["total_tokens"] = _four_class_total([{
        "values": {field: deltas[field] for field in TOKEN_CLASSES},
        "relation": "delta",
    }], empty=0) if all(deltas[field] is not None for field in TOKEN_CLASSES) else None

    return [{
        "completion_state": {"left": result["left_completion"], "right": result["right_completion"]},
        "context_features": {"left": result["left_context"], "right": result["right_context"]},
        "delegation_metrics": {"left": result["left_delegation"], "right": result["right_delegation"]},
        "resource_metrics": {"left": result["left_resources"], "right": result["right_resources"]},
        "state_change_metrics": {"left": result["left_changes"], "right": result["right_changes"]},
        "token_deltas": deltas,
        "tool_metrics": {"left": result["left_tools"], "right": result["right_tools"]},
        "turn_call_counts": {"left": result["left_turn_calls"], "right": result["right_turn_calls"]},
    }]
...
def _grades(case: Mapping[str, Any], rows: Sequence[Mapping[str, Any]]) -> dict[str, str]:
    ...
    if (
        ...
        or any(not isinstance(field, str) or not isinstance(grade, str) for field, grade in expected.items())
    ):
...
def _total_order(case: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for fact in _ordered(_facts(case)):
        ...
        if not isinstance(coordinates, Mapping):
            continue
        ordered.append({"logical_id": fact.get("logical_id"), "relation": fact.get("relation"), "coordinate": dict(coordinates)})
    return ordered
    ...
    if kind == "model_profile":
        ...
        profile = next(
            (
                fact.get("values")
                for fact in facts
                if fact.get("relation") == "model_profile"
                and fact.get("logical_id") == logical_id
            ),
            None,
        )
...
def _publication_provenance(
    facts: Sequence[Mapping[str, Any]], logical_id: str
) -> dict[str, Any]:
    for fact in facts:
...
def _provenance(
    ...
    if (
        ...
        or any(field not in provenance or provenance[field] in (None, "", [], {}) for field in required)
    ):
...
def evaluate_wf02_events(events: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    for event in events:
        ...
        if not isinstance(coordinate[0], bool) or any(isinstance(coordinate[index], bool) or not isinstance(coordinate[index], int) for index in (1, 2, 3, 4, 6)) or not isinstance(coordinate[5], str) or not coordinate[5]:
    ...
    success = min(success_candidates, key=lambda item: tuple(item["coordinate"])) if success_candidates else None
    mutation_candidates = [event for event in records if event["kind"] == "state_change"]
    mutation = min(mutation_candidates, key=lambda item: tuple(item["coordinate"])) if mutation_candidates else None
    calls = [event for event in records if event["kind"] == "call"]
    def sum_before(boundary: Mapping[str, Any] | None) -> int | None:
        if boundary is None:
            return None
        prior = [event for event in calls if tuple(event["coordinate"]) < tuple(boundary["coordinate"])]
        if not prior:
            return 0
        total = 0
        for event in prior:
            tokens = event.get("tokens")
            if not isinstance(tokens, list) or len(tokens) != 4 or any(item is None for item in tokens):
                return None
            ...
            total += sum(tokens)
        return total
    result = {"first_action_tokens": sum_before(action), "first_success_tokens": sum_before(success), "first_mutation_tokens": sum_before(mutation), "mutation_observed": mutation is not None}
    if action is not None and len(starts) > 1:
        action_shape = tuple(action["coordinate"][:5]) + (action["coordinate"][6],)
        if any(tuple(item["coordinate"][:5]) + (item["coordinate"][6],) == action_shape for item in starts.values() if item is not action):
    ...
    if success is not None and len(success_candidates) > 1:
        ...
        if any(tuple(item["coordinate"][:5]) + (item["coordinate"][6],) == success_shape for item in success_candidates if item is not success):
...
def load_cases(path: Path = SCENARIOS_PATH) -> list[Mapping[str, Any]]:
    ...
    if not isinstance(value, Mapping) or value.get("schema") != "codex-usage-tracker.synthetic-question-scenarios.v1":
