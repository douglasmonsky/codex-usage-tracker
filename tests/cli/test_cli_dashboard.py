...
def test_serve_dashboard_json_reports_react_url_and_legacy_fallback(
    ...
    assert served["refresh_on_start"] is False


def test_serve_dashboard_refreshes_in_background(
    tmp_path: Path,
    monkeypatch,
    capsys,
) -> None:
    ...
    payload = json.loads(capsys.readouterr().out)
    assert payload["refresh_before_start"] is False
    assert payload["refresh_in_background"] is True
    assert served["refresh_on_start"] is True
