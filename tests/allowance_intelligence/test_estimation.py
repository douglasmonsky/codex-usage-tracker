...
def _cycle(cycle_id: str, ended: str, *, used: float = 50) -> dict[str, object]:
    return {
        "cycle_id": cycle_id,
        "window_kind": "weekly",
...
def _interval(cycle_id: str, ended: str, *, start: float, end: float, credits: float | None = 100, coverage: float | None = 1) -> dict[str, object]:
    return {
        "cycle_id": cycle_id,
        "window_kind": "weekly",
...
def test_estimates_are_prior_only_and_future_rows_do_not_change_history() -> None:
    cycles = [_cycle("one", "2026-07-01T00:00:00+00:00"), _cycle("two", "2026-07-08T00:00:00+00:00")]
    intervals = [
    ...
    after = build_weekly_estimation(cycles + [_cycle("future", "2026-07-20T00:00:00+00:00")], intervals + [_interval("future", "2026-07-19T00:00:00+00:00", start=0, end=50, credits=1000)], now=NOW)
...
def test_missing_pricing_is_an_explicit_coverage_gap_not_imputed() -> None:
    result = build_weekly_estimation(
        ...
        [_interval("one", "2026-06-30T00:00:00+00:00", start=0, end=50, credits=None, coverage=None)],
        now=NOW,
    )
...
def test_sufficient_history_calculates_walk_forward_validation_and_pace_scenarios() -> None:
    cycles = [_cycle(f"cycle-{index}", f"2026-07-15T{8 + index:02d}:30:00+00:00") for index in range(4)]
    intervals = [
        _interval(f"cycle-{index}", f"2026-07-15T{8 + index:02d}:00:00+00:00", start=0, end=50, credits=100)
    ...
    assert set(validation["baselines"]) == {"unchanged_counter", "previous_interval", "recent_observed_pace", "previous_cycle_pace"}
