...
export type EvidenceConsoleRoutePlacement = 'primary' | 'contextual' | 'utility';
...
export function evidenceConsoleRouteDefinition(
  routeId: EvidenceConsoleRouteId,
): EvidenceConsoleRouteDefinition {
  const definition = evidenceConsoleRoutes.find(route => route.id === routeId);
  if (!definition) throw new Error(`Unknown Evidence Console route: ${routeId}`);
  return definition;
}
