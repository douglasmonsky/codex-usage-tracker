...
export function InvestigatorPage({ model }: { model: DashboardModel }) {
  const selected = model.findings[0];
  ...
  return (
    <div className="workbench-layout">
      ...
      <Panel title="What Is Driving Usage?" subtitle="Ranked by estimated credit impact">
        <div className="finding-list">
          {model.findings.map(finding => (
            <FindingCard key={finding.rank} finding={finding} />
          ))}
        </div>
      </Panel>
      <div className="stacked-panels">
        ...
        <Panel title="Evidence Table" subtitle={`${formatNumber(model.calls.length)} preview calls`}>
          <DataTable columns={callColumns.slice(0, 8)} data={model.calls} compact />
        </Panel>
      </div>
      {selected ? <SelectedFinding finding={selected} /> : null}
    </div>
  );
}
...
function SelectedFinding({ finding }: { finding: Finding }) {
  return (
    <aside className="side-panel">
      <Panel title="Selected Finding" subtitle={finding.title}>
        ...
        <button className="primary-button stretch" type="button">
