...
- [ ] Construct intervals by retaining canonical calls while percentage is unchanged, closing a positive interval only at a later accepted increase, and censoring pending attribution on reset, cohort/window change, unresolved conflict, or invalid weekly decrease. Preserve canonical local activity between valid anchors even when intermediate observations are missing; never use alternate snapshots as normal-cohort boundaries.
- [ ] Populate structural eligibility flags conservatively: interpolation may be eligible only inside a non-ambiguous same-cohort weekly cycle with a positive increase, positive local activity, stable reset identity, and no conflict/reversal/censor reason. Leave pricing-dependent calibration/forecast/change eligibility disabled until later enrichment proves at least 95% supported priced-credit coverage.
...
- [ ] Write failing tests for latest status lookup, bounded range lookup, archive inclusion/exclusion, newest/oldest evidence ordering, window/cohort/global range filtering, nullable evidence timestamps, cursor continuation, malformed cursor shape, cursor revision mismatch, and query plans using the new indexes.
...
- [ ] Add migration 27 with exact indexes for active/archive-partitioned latest, series, and evidence queries across window+cohort, window-only, cohort-only, and global evidence scopes. For `include_archived=True`, issue one indexed query per archive partition and linearly merge the two already ordered streams in O(k); do not re-sort results or sacrifice index order to an optional predicate.
