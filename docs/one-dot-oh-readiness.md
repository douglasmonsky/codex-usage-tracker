...
- [x] Keep the post-publish public PyPI version check aligned to release target `0.7.0`: `python -c "import json, urllib.request; print(json.load(urllib.request.urlopen('https://pypi.org/pypi/codex-usage-tracking/json'))['info']['version'])"`.
- [x] Keep the post-publish public venv install smoke aligned to release target `0.7.0`: `python -m venv /tmp/codex-usage-pypi-smoke && . /tmp/codex-usage-pypi-smoke/bin/activate && python -m pip install codex-usage-tracking==0.7.0 && codex-usage-tracker --version`.
- [x] Keep the post-publish public pipx install smoke aligned to release target `0.7.0`: `PIPX_HOME=/tmp/codex-usage-pipx-home PIPX_BIN_DIR=/tmp/codex-usage-pipx-bin pipx install codex-usage-tracking==0.7.0 && /tmp/codex-usage-pipx-bin/codex-usage-tracker --version`.
...
- [x] Keep the post-publish public PyPI Docker smoke aligned to release target `0.7.0`: `python scripts/smoke_installed_package.py --docker --from-pypi --version 0.7.0`.
...
These references are the concrete proof behind completed checklist items. Public package smoke commands are kept version-specific to release target `0.7.0` and should be run after publication; all repo tests use synthetic or aggregate-only data.

### Public Install And Package Metadata

- Public PyPI version, public venv install, and public pipx install command drift is guarded by the exact post-publish public-install commands in section 1.
...
- Public PyPI Docker command drift is guarded by `scripts/smoke_installed_package.py --docker --from-pypi --version 0.7.0`.
...
- TestPyPI publish process should be proven after publication by a workflow-dispatch run on `main`, followed by TestPyPI metadata and clean virtualenv install checks for `codex-usage-tracking==0.7.0`.
- PyPI publish process should be proven after publication by a workflow-dispatch run on `main`, protected `pypi` environment approval, PyPI metadata visibility, clean virtualenv install, temporary pipx install, and Docker public-package smoke for `codex-usage-tracking==0.7.0`.
