...
- Session rows show started time, ended time, start reason, idle time before the detected boundary, duration, call count, total tokens, uncached input, average cache ratio, largest uncached miss, and context peak.
- `Idle before` is the elapsed time between the first call in that session and the immediately previous call in the same resolved thread. It is populated only when a cold-resume boundary is detected; thread-start rows show `-`.
- Click a session row to expand the calls in that work session. The expanded panel also shows aggregate context segments split at detected post-compaction boundaries, with token/cache totals, context peak, and first post-compaction uncached miss, without loading raw transcript evidence.
...
- Click headers such as `Started`, `Ended`, `Source`, `Idle before`, `Uncached`, `Largest miss`, or `Cache` to sort server-backed session rows.
