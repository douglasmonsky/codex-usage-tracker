...
> Historical record only. This ledger governed the runtime compatibility window
> before the Product Kernel Reset. The active compatibility and removal contract
> is [`docs/deprecations.md`](../../../deprecations.md).

This was the normative compatibility ledger for the
[MCP-first product pivot](roadmap.md). Every deprecated public surface had an
owner through its final supported release, a deterministic compatibility test,
and a concrete migration example.
