...
### `refactor/piecewise-adaptation-position`
Status:
- Local branch only. Not pushed.
- Green checkpoint reached.

Objective:
- Reduce `_piecewise_adaptation_by_position` complexity while preserving position-bucket aggregation by segment label.
- Add direct characterization coverage for all-segment and per-label position reports.
...
Completed edits:
- Split empty position buckets, segment row collection, row append behavior, and report assembly into focused helpers.
- Preserved `position_buckets`, `all_segments`, `by_label`, prediction-row counts, and best-by-MAE output.

Metrics:
- `_piecewise_adaptation_by_position`: C(12) -> A(2).
- `usage_drain_regime_segments.py` maximum complexity: B(8).
- `usage_drain_regime_segments.py`: 369 -> 411 physical lines.
- `tests/test_usage_drain_regime_segments.py`: 48 -> 86 physical lines.
- Global C-or-worse blocks: 22 -> 21.
- No C(13) or D/E/F complexity blocks remain.
- Current top complexity target is `usage_drain_error_diagnostics.py::span_error_metadata` at C(12).

Checks so far:
- `.venv/bin/python -m pytest tests/test_usage_drain_regime_segments.py -q`: 2 passed.
- `.venv/bin/python -m pytest tests/test_usage_drain_regime_segments.py tests/test_usage_drain_model.py tests/test_usage_drain_reports.py -q`: 21 passed.
- `.venv/bin/python -m ruff check src/codex_usage_tracker/usage_drain_regime_segments.py tests/test_usage_drain_regime_segments.py`: passed.
- `.venv/bin/python -m mypy`: passed.
- `.venv/bin/radon cc src/codex_usage_tracker/usage_drain_regime_segments.py -a -s`: target now A(2), module max B(8).
...
### `refactor/boundary-delta-residual-diagnostics`
Status:
- Local branch only. Not pushed.
- Green checkpoint reached; ready for local commit.

Objective:
- Reduce `_boundary_delta_residual_diagnostics` complexity while preserving residual summary, grouped error, and largest-error payloads.
- Add direct characterization coverage for residual diagnostic summary shares, top transition groups, and largest-error row formatting.
...
Completed edits:
- Split residual error-row construction, empty residual payload, summary share calculation, grouped error mapping, and largest-error assembly behind focused helpers.
- Preserved existing residual diagnostic JSON keys and ordering-sensitive largest-error behavior.

Metrics:
- `_boundary_delta_residual_diagnostics`: C(12) -> A(2).
- `usage_drain_boundary_delta_summary.py` maximum complexity: C(11), now `_boundary_delta_risk_gate_diagnostics`.
- `usage_drain_boundary_delta_summary.py`: 421 -> 457 physical lines.
- `tests/test_usage_drain_boundary_delta_summary.py`: 154 -> 231 physical lines.
- Global C-or-worse blocks: 20 -> 19.
- No C(13) or D/E/F complexity blocks remain.
...
Checks:
- `.venv/bin/python -m pytest tests/test_usage_drain_boundary_delta_summary.py -q`: 4 passed.
- `.venv/bin/python -m pytest tests/test_usage_drain_boundary_delta_summary.py tests/test_usage_drain_model.py -q`: 18 passed.
- `.venv/bin/python -m ruff check src/codex_usage_tracker/usage_drain_boundary_delta_summary.py tests/test_usage_drain_boundary_delta_summary.py`: passed.
- `.venv/bin/python -m mypy`: passed.
- `.venv/bin/radon cc src/codex_usage_tracker/usage_drain_boundary_delta_summary.py -a -s`: target now A(2), module max C(11).
- `.venv/bin/python -m pytest -q`: 493 passed.
- `.venv/bin/python -m compileall src`: passed.
- `.venv/bin/python -m ruff check .`: passed.
- `.venv/bin/tach check`: passed.
- `.venv/bin/python scripts/check_release.py`: passed.
...
### `refactor/diagnostic-snapshot-report-render`
Status:
- Local branch only. Not pushed.
- Green checkpoint reached; ready for local commit.

Objective:
- Reduce `DiagnosticSnapshotReport.render` below the C-grade complexity threshold without changing CLI snapshot report output selection.
- Add focused coverage for section dispatch before replacing the branch chain.

Files touched:
- `src/codex_usage_tracker/diagnostic_snapshot_report.py`
- `tests/test_diagnostic_snapshot_report.py`
- `docs/maintainability-roadmap.md`

Completed edits:
- Added focused tests covering all diagnostic snapshot section headings, overview fallback, and unavailable snapshot text.
- Replaced the long `render` section branch chain with a bound-method dispatch table.
- Extracted unavailable snapshot text into a helper while keeping the public report API unchanged.

Metrics:
- `DiagnosticSnapshotReport.render`: C(11) -> A(2).
- `diagnostic_snapshot_report.py` maximum complexity: A(5); no C-grade functions remain in module.
- Global C-or-worse blocks: 13 -> 12.
- No C(12+) or D/E/F complexity blocks remain.

Checks:
- `.venv/bin/python -m pytest tests/test_diagnostic_snapshot_report.py -q`: 10 passed.
- `.venv/bin/python -m ruff check src/codex_usage_tracker/diagnostic_snapshot_report.py tests/test_diagnostic_snapshot_report.py`: passed.
- `.venv/bin/radon cc src/codex_usage_tracker/diagnostic_snapshot_report.py -a -s`: target now A(2), module max A(5).
- `.venv/bin/python -m mypy`: passed.
- `.venv/bin/python -m pytest -q`: 513 passed.
- `.venv/bin/python -m compileall src`: passed.
- `.venv/bin/python -m ruff check .`: passed.
- `.venv/bin/tach check`: passed.
- `.venv/bin/python scripts/check_release.py`: passed.
...
### `refactor/store-schema-init-db`
Status:
- Local branch only. Not pushed.
- Green checkpoint reached; ready for local commit.

Objective:
- Reduce `store_schema.py::init_db` below the C-grade complexity threshold without changing migration order, repair behavior, or schema version semantics.
- Add direct coverage for fresh-database migration history.

Files touched:
- `src/codex_usage_tracker/store_schema.py`
- `tests/test_store_migrations.py`
- `docs/maintainability-roadmap.md`

Completed edits:
- Added focused coverage proving a fresh database records all schema migrations and sets `PRAGMA user_version` to the current schema version.
- Replaced the repeated migration branch chain with a data-driven migration list and a single migration application helper.
- Preserved the existing behavior that migrations run idempotently for both old and current `user_version` values while recording missing migration metadata.

Metrics:
- `init_db`: C(11) -> A(2).
- `store_schema.py` maximum complexity: B(6); no C-grade functions remain in module.
- Global C-or-worse blocks: 9 -> 8.
- No C(12+) or D/E/F complexity blocks remain.

Checks:
- `.venv/bin/python -m pytest tests/test_store_migrations.py -q`: 6 passed.
- `.venv/bin/python -m ruff check src/codex_usage_tracker/store_schema.py tests/test_store_migrations.py`: passed.
- `.venv/bin/radon cc src/codex_usage_tracker/store_schema.py -a -s`: target now A(2), module max B(6).
- `.venv/bin/python -m mypy`: passed.
...
### `refactor/prediction-error-diagnostics`
...
Objective:
- Reduce the C-grade prediction error diagnostics helper while preserving error-share, grouping, and largest-error payloads.
...
Completed edits:
- Added synthetic characterization tests for populated and empty prediction error diagnostics.
- Split prediction error row construction, empty payload, summary assembly, and threshold-share calculation into focused helpers.
- Preserved diagnostic keys, transition groups, date groups, and largest-error metadata.

Metrics:
- `prediction_error_diagnostics`: C(11) -> A(2).
- `usage_drain_error_diagnostics.py` maximum complexity: B(9); no C-grade functions remain in module.
- Expected global C-or-worse blocks after this branch: 4 -> 3.
- No C(12+) or D/E/F complexity blocks expected.

Checks:
- `PYTHONPATH=src .venv/bin/python -m pytest tests/test_usage_drain_error_diagnostics.py -q`: 3 passed before refactor.
- `PYTHONPATH=src .venv/bin/python -m pytest tests/test_usage_drain_error_diagnostics.py tests/test_usage_drain_model.py -q`: 17 passed.
- `.venv/bin/python -m ruff check src/codex_usage_tracker/usage_drain_error_diagnostics.py tests/test_usage_drain_error_diagnostics.py`: passed.
- `.venv/bin/radon cc src/codex_usage_tracker/usage_drain_error_diagnostics.py -a -s`: target now A(2), module max B(9).
- `.venv/bin/python -m mypy`: passed.
- `.venv/bin/python -m pytest -q`: 528 passed.
- `.venv/bin/python -m compileall src`: passed.
- `.venv/bin/python -m ruff check .`: passed.
- `.venv/bin/tach check`: passed.
- `.venv/bin/python scripts/check_release.py`: passed.
