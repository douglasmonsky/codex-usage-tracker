...
def build_weekly_estimation(
    ...
    weekly_cycles = sorted(
        (row for row in cycles if row.get("window_kind") == "weekly" and str(row.get("last_observed_at") or "") <= as_of),
    ...
    weekly_intervals = sorted(
        (row for row in intervals if row.get("window_kind") == "weekly" and str(row.get("end_observed_at") or "") <= as_of),
        key=lambda row: (str(row.get("end_observed_at") or ""), str(row.get("cycle_id") or "")),
    )
    quality_cycles = [row for row in weekly_cycles if _quality_cycle(row)]
    ...
    for interval in weekly_intervals:
        ...
        cumulative = _cumulative_credits(weekly_intervals, cycle_id, str(interval.get("end_observed_at") or ""))
        explained = credits / prior_capacity if prior_capacity and prior_capacity > 0 else None
        estimated = start + explained * cumulative / credits if start is not None and explained is not None and credits > 0 else None
        ...
        if cycle_capacity is not None and cycle_id not in {item[0] for item in completed}:
            ...
            is_last = str(interval.get("end_observed_at") or "") == _last_interval_at(weekly_intervals, cycle_id)
            if is_last and _quality_cycle(cycle_by_id.get(cycle_id, {})):
    ...
    return {
        ...
        "coverage_gaps": {"missing_pricing_interval_count": missing_pricing, "eligible_interval_count": eligible_intervals},
...
def _quality_cycle(row: dict[str, Any]) -> bool:
    return row.get("quality_grade") in {"high", "medium"} and row.get("status") == "accepted" and row.get("cycle_state") == "accepted"
...
def _capacity_by_cycle(intervals: list[dict[str, Any]], cycles: dict[str, dict[str, Any]]) -> dict[str, float]:
    result: dict[str, float] = {}
    for cycle_id, cycle in cycles.items():
        if not _quality_cycle(cycle): continue
        rows = [row for row in intervals if str(row.get("cycle_id")) == cycle_id and _eligible(row)]
        credits = sum(value for row in rows if (value := _number(row.get("estimated_credits"))) is not None and (_number(row.get("price_coverage")) or 0) > 0)
        delta = sum(max(0.0, _number(row.get("visible_percent_delta")) or 0.0) for row in rows)
        if credits > 0 and delta > 0: result[cycle_id] = credits / delta
    return result
def _cumulative_credits(intervals: list[dict[str, Any]], cycle_id: str, ended: str) -> float:
    return sum(_number(row.get("estimated_credits")) or 0.0 for row in intervals if str(row.get("cycle_id")) == cycle_id and str(row.get("end_observed_at") or "") <= ended)
def _last_interval_at(intervals: list[dict[str, Any]], cycle_id: str) -> str:
    return max((str(row.get("end_observed_at") or "") for row in intervals if str(row.get("cycle_id")) == cycle_id), default="")
def _total_ratio(by_cycle: dict[str, float], completed: list[tuple[str, float]]) -> float | None: return _weighted_median([by_cycle[key] for key, _ in completed])
def _unexplained_share(rows: list[dict[str, Any]]) -> float | None:
    corrections = [abs(_number(row.get("anchor_correction")) or 0.0) for row in rows if row.get("anchor_correction") is not None]
    observed = [abs(_number(row.get("estimated_used_percent")) or 0.0) for row in rows if row.get("anchor_correction") is not None]
...
def _prior_errors(rows: list[dict[str, Any]]) -> dict[str, float | int | None]:
    errors = [abs(_number(row.get("anchor_correction")) or 0.0) for row in rows if row.get("anchor_correction") is not None]
    return {"sample_size": len(errors), "median_absolute_error": _round(median(errors)) if errors else None}
def _weekly_estimate(row: dict[str, Any] | None, status: str) -> dict[str, Any]:
    if status != "validated" or row is None or row.get("estimated_used_percent") is None: return {"used_percent": None, "clipped": False, "reason": "insufficient_prior_capacity"}
    raw = float(row["estimated_used_percent"]); clipped = min(100.0, max(0.0, raw))
    return {"used_percent": _round(clipped), "clipped": clipped != raw, "reason": None}
def _forecast(estimate: dict[str, Any], status: str, count: int, validation: dict[str, Any]) -> dict[str, Any]:
    if status != "validated" or estimate["used_percent"] is None: return {"used_percent": None, "reason": "insufficient_prior_cycle_evidence", "quantiles": None}
    residuals = validation["residual_quantiles"]
    center = float(estimate["used_percent"])
    return {"used_percent": estimate["used_percent"], "reason": None, "quantiles": {"p10": _round(min(100, max(0, center + residuals["p10"]))), "p50": _round(min(100, max(0, center + residuals["p50"]))), "p90": _round(min(100, max(0, center + residuals["p90"])))}, "sample_size": count}
def _validation(rows: list[dict[str, Any]], status: str) -> dict[str, Any]:
    residuals = [_number(row.get("anchor_correction")) for row in rows if row.get("anchor_correction") is not None]
    absolute = [abs(value) for value in residuals if value is not None]
    quantiles = {"p10": _quantile(residuals, .1), "p50": _quantile(residuals, .5), "p90": _quantile(residuals, .9)}
    coverage = {str(level): _round(sum(abs(value) <= _quantile([abs(item) for item in residuals], level / 100) for value in residuals) / len(residuals)) if residuals else None for level in (50, 80, 95)}
    mae = sum(absolute) / len(absolute) if absolute else None
    baseline_errors = {"unchanged_counter": [_number(row.get("observed_delta")) or 0.0 for row in rows], "previous_interval": [abs((_number(row.get("observed_delta")) or 0.0) - (_number(rows[index - 1].get("observed_delta")) or 0.0)) for index, row in enumerate(rows) if index], "recent_observed_pace": [abs((_number(row.get("observed_delta")) or 0.0) - median([_number(item.get("observed_delta")) or 0.0 for item in rows[:index]])) for index, row in enumerate(rows) if index], "previous_cycle_pace": [abs((_number(row.get("observed_delta")) or 0.0) - (_number(rows[index - 1].get("observed_delta")) or 0.0)) for index, row in enumerate(rows) if index]}
    baselines = {key: {"mean_absolute_error": _round(sum(values) / len(values)) if values else None, "sample_size": len(values)} for key, values in baseline_errors.items()}
    beats = mae is not None and all(item["mean_absolute_error"] is None or mae <= item["mean_absolute_error"] for item in baselines.values())
    validated = status == "validated" and len(residuals) >= 3 and beats and (coverage["80"] or 0) >= .5
    return {"status": "validated" if validated else "descriptive", "sample_size": len(residuals), "evaluation_horizon": "walk_forward", "calibration_window": "prior_completed_cycles", "median_absolute_error": _round(median(absolute)) if absolute else None, "mean_absolute_error": _round(mae), "rmse": _round((sum(error * error for error in absolute) / len(absolute)) ** .5) if absolute else None, "residual_quantiles": quantiles, "interval_coverage": coverage, "segmented_errors": {"weekly": {"mean_absolute_error": _round(mae), "sample_size": len(absolute)}}, "baselines": baselines}
...
def _pace_scenarios(intervals: list[dict[str, Any]], now: Any) -> dict[str, Any]:
    ...
    windows = {"recent_6h": rate(recent6), "trailing_24h": rate(recent24), "current_cycle": rate(current), "comparable_prior_cycle": rate(prior)}
    values = [value for value in windows.values() if value is not None]
    if len(values) < 4: return {"status": "observed_only", "reason": "insufficient_recent_pace_samples", "if_current_pace_continues": None, "contributing_windows": [key for key, value in windows.items() if value is not None], "sample_count": len(values)}
    center = median(values)
    return {"status": "conditional", "reason": None, "if_current_pace_continues": _round(center), "low": _round(_quantile(values, .1)), "high": _round(_quantile(values, .9)), "contributing_windows": windows, "sample_count": len(values)}
def _age_hours(row: dict[str, Any], now: Any) -> float:
    from datetime import datetime
    return max(0, (now - datetime.fromisoformat(str(row.get("end_observed_at")).replace("Z", "+00:00"))).total_seconds() / 3600)
