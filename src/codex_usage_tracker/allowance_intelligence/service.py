...
def build_allowance_status(connection: sqlite3.Connection, *, now: datetime, privacy_mode: str = "strict", include_archived: bool = False, since_revision: str | None = None) -> dict[str, Any]:
    ...
    partial = any(entry and entry["status"] == "conflict" for entry in windows.values())
    data_state = "partial" if partial else _data_state(states)
    return {"schema": ALLOWANCE_STATUS_SCHEMA, "model_version": MODEL_VERSION, "generated_at": now.isoformat(), "data_as_of": _latest_at(windows), "revision": revision, "changed": True, "privacy_mode": privacy_mode, "include_archived": include_archived, "data_state": data_state, "weekly": windows["weekly"], "five_hour": windows["five_hour"], "quality": {"canonical": True, "copied_rows_excluded": _copied_excluded(connection)}, "cohorts": {"selected": _cohort(weekly), "alternates": []}, "next": {"action": "poll_status", "poll_after_seconds": 30 if data_state in {"fresh", "aging", "partial"} else 60}}
...
def build_allowance_series(connection: sqlite3.Connection, *, now: datetime, range_preset: str = "7d", start_at: str | None = None, end_at: str | None = None, granularity: str = "auto", window_kind: str = "weekly", cohort_id: str | None = None, include_archived: bool = False) -> dict[str, Any]:
    ...
    points = [{"kind": "observed", "cycle_id": row["cycle_id"], "observed_at": row["last_observed_at"], "used_percent": row["latest_used_percent"], "reset_at": row["reset_at"]} for row in cycles]
...
def build_allowance_evidence(connection: sqlite3.Connection, *, now: datetime | None = None, privacy_mode: str = "strict", limit: int = 50, cursor: str | None = None, window_kind: str | None = None, cohort_id: str | None = None, include_archived: bool = False) -> dict[str, Any]:
    ...
    page = query_allowance_evidence(connection, limit=limit, cursor=cursor, window_kind=window_kind, cohort_id=cohort_id, include_archived=include_archived)
    rows = [_evidence_row(row, privacy_mode) for row in page.rows if row.get("point_kind") in {"positive", "conflict", "censored"}]
    return {"schema": ALLOWANCE_EVIDENCE_SCHEMA, "model_version": MODEL_VERSION, "generated_at": (now or datetime.now(timezone.utc)).isoformat(), "revision": _revision(connection), "privacy_mode": privacy_mode, "rows": rows, "next_cursor": page.next_cursor, "copied_rows_excluded": _copied_excluded(connection), "provenance": "local" if privacy_mode != "strict" else "local_aggregate", "offline_export_action": "build_allowance_export_report"}
