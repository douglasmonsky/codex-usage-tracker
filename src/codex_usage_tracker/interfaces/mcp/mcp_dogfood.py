"""Async dogfood job state and cache helpers for MCP implementations."""
...
from codex_usage_tracker.interfaces.mcp.serialization import (
    copy_json_object,
    pretty_json,
)
...
def load_cached_result(
    ...
    if cached is not None:
        return copy_json_object(cached["result"]), "memory"
    ...
    _prune_result_cache()
    return copy_json_object(payload), "disk"
...
def _store_cached_result(params: dict[str, Any], payload: dict[str, Any]) -> str:
    ...
    DOGFOOD_RESULT_CACHE[result_cache_key] = {
        "stored_at": utc_now(),
        "result": copy_json_object(payload),
    }
    _prune_result_cache()
    path = _cache_path(params, result_cache_key)
...
def job_status_payload(
    ...
    *,
    ...
    with DOGFOOD_JOB_LOCK:
        ...
        return copy_json_object(payload)
