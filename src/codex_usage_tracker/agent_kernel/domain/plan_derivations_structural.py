...
def _derive_first_action_mutation_v1(
    ...
    for fact in tools:
        ...
        if not _window_contains(request, start):
            continue
        ...
        tool_boundaries.append((start, "tool_start", _stable_id(fact, "tool_id")))
        ...
        if terminal is not None:
            ...
            if lifecycle == "succeeded" and not _window_contains(request, terminal):
                continue
            if lifecycle == "succeeded":
                tool_boundaries.append(
                    (terminal, "tool_terminal_succeeded", _stable_id(fact, "tool_id"))
                )
...
def _unbound_call(use: Mapping[str, Any], operands: Mapping[str, Any]) -> FormulaInvocation:
    call = _call(use, operands)
    return FormulaInvocation(
        call.use_id,
        call.formula_id,
